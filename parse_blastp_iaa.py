...
def parseblastout2(accession):
    ...
    if sum(gene in list(genome_match['qseqid']) for gene in ["IaaP","IaaQ","IaaR","IaaA","IaaB","IaaC","IaaD","IaaE","IaaF","IaaG","IaaH","IaaI","IaaJ","IaaK","IaaL","IaaM"]) >= 12:
...
if not os.path.exists(output_directory+"iaa_positive_df.pickle"):
    ...
    blastout_filtered = blastout[(blastout["qseqid"].isin(["IaaP","IaaQ","IaaR","IaaA","IaaB","IaaC","IaaD","IaaE","IaaF","IaaG","IaaH","IaaI","IaaJ","IaaK","IaaL","IaaM"])) &
            (blastout['evalue'] <= 1e-10) & (blastout['bitscore']>=50)]
    #print raw blast out statistics:
    iaa_proteins = ["IaaP","IaaQ","IaaR","IaaA","IaaB","IaaC","IaaD","IaaE","IaaF","IaaG","IaaH","IaaI","IaaJ","IaaK","IaaL","IaaM"]
    raw_blast_stats=[]
    for i in iaa_proteins:
    ...
    print('\nObtaining iaa cluster positive data frame')
    ...
    iaa_positive_df['has_overlap'] = iaa_positive_df['synteny_dir_dist'].str.contains("--")
    iaa_positive_df['duplicated']= iaa_positive_df.duplicated(subset="filename")
    print('Writing positive accessions to file')
    assembly_list = "\n".join(list(set(iaa_positive_df['assembly']))) + "\n"
    with open(output_directory+"iaa_positive_accessions.txt",'w+') as output:
        output.writelines(assembly_list)
    print('Fetching genome metadata')
    all_accs=list(set(iaa_positive_df['accession']))
    all_biosamples = list(iaa_positive_df['biosample'])
    ...
    iaa_positive_df['isolation_src'] = iaa_positive_df['biosample'].map(isolation_source_dict)
    iaa_positive_df['env_biome'] = iaa_positive_df['biosample'].map(env_biome_dict)
    iaa_positive_df['env_feature'] = iaa_positive_df['biosample'].map(env_feature_dict)
    iaa_positive_df['host'] = iaa_positive_df['biosample'].map(host_dict)
    iaa_positive_df['host_sciname'] = iaa_positive_df['biosample'].map(host_sciname_dict)
    iaa_positive_df['strain'] = iaa_positive_df['biosample'].map(strain_dict)
    iaa_positive_df['all_biosample_metadata'] = iaa_positive_df['biosample'].map(all_meta_dict)
    all_assemblies= ['RS_'+ i for i in list(set(iaa_positive_df['assembly']))]
    ...
    iaa_positive_df['gtdb_tax'] = iaa_positive_df['assembly'].map(gtdb_dict)
    iaa_positive_df['ncbi_tax'] = iaa_positive_df['assembly'].map(ncbi_dict)
    iaa_positive_df['same_taxonomy'] = iaa_positive_df['gtdb_tax'] == iaa_positive_df['ncbi_tax']
    iaa_positive_df[['domain_gtdb','phylum_gtdb','class_gtdb','order_gtdb','family_gtdb','genus_gtdb','species_gtdb']]=iaa_positive_df['gtdb_tax'].str.split(";", expand = True)
    print('\nWriting iaa cluster positive data frame to file')
    iaa_positive_df.to_csv(output_directory+"iaa_positive_df.tsv",sep = '\t', index = False)
    iaa_positive_df.to_pickle(output_directory+"iaa_positive_df.pickle")

iaa_positive_df=pd.read_pickle(output_directory+"iaa_positive_df.pickle")
inputs_indexprot = [re.sub('.gbff','_proteins.fa', i) for i in list(set(iaa_positive_df['filename']))]
...
inputs_fetchneighborhood = list(range(0,len(iaa_positive_df)))
...
iaa_pos_neighborhoods = list(filter(None, outputs_fetchneighborhood))
iaa_pos_neighborhoods_df= pd.DataFrame(iaa_pos_neighborhoods, columns=fetchneighborhood_columns)
iaa_pos_neighborhoods_df.to_csv(output_directory+"iaa_pos_neighborhoods.tsv",sep = '\t', index = False)
iaa_pos_neighborhoods_df.to_pickle(output_directory+"iaa_pos_neighborhoods.pickle")
# merge the neighborhoods and iaa_positive dataframes
print('\nMerging all data and writing it to file')
iaa_positive_df = pd.read_pickle(output_directory+"iaa_positive_df.pickle")
iaa_pos_neighborhoods_df = pd.read_pickle(output_directory+"iaa_pos_neighborhoods.pickle")
iaa_positive_all = pd.merge(iaa_pos_neighborhoods_df,iaa_positive_df, on = ["accession","cluster_number","assembly"])
iaa_positive_all = iaa_positive_all.rename(columns={"coord_list_y": "coord_list"}).drop(columns=['coord_list_x'])
iaa_positive_all_gtdb= iaa_positive_all[iaa_positive_all['gtdb_tax'].notnull()]
iaa_positive_all.to_pickle(output_directory+"iaa_positive_all_data.pickle")
iaa_positive_all.to_csv(output_directory+"iaa_positive_all_data.tsv", sep = '\t', index = False)
iaa_positive_all.to_excel(output_directory+"iaa_positive_all_data.xlsx", index = False)
iaa_positive_all_gtdb.to_pickle(output_directory+"iaa_positive_all_data_gtdb.pickle")
iaa_positive_all_gtdb.to_csv(output_directory+"iaa_positive_all_data_gtdb.tsv", sep = '\t', index = False)
iaa_positive_all_gtdb.to_excel(output_directory+"iaa_positive_all_data_gtdb.xlsx", index = False)


#get 10 neighbors on each side:

# inputs_fetchneighborhood_10_10 = list(range(0,len(iaa_positive_df)))
...
#         pass
#
# iaa_pos_neighborhoods_10_10 = list(filter(None, outputs_fetchneighborhood_10_10))
# iaa_pos_neighborhoods_df_10_10= pd.DataFrame(iaa_pos_neighborhoods_10_10, columns=fetchneighborhood_columns)
# iaa_pos_neighborhoods_df_10_10.to_csv(output_directory+"iaa_pos_neighborhoods_10_10.tsv",sep = '\t', index = False)
# iaa_pos_neighborhoods_df_10_10.to_pickle(output_directory+"iaa_pos_neighborhoods_10_10.pickle")
# iaa_pos_neighborhoods_df_10_10 = pd.read_pickle(output_directory+"iaa_pos_neighborhoods_10_10.pickle")
# iaa_positive_all_10_10 = pd.merge(iaa_pos_neighborhoods_df_10_10,iaa_positive_df, on = ["accession","cluster_number","assembly"])
# iaa_positive_all_10_10 = iaa_positive_all_10_10.rename(columns={"coord_list_y": "coord_list"}).drop(columns=['coord_list_x'])
# iaa_positive_all_gtdb_10_10= iaa_positive_all_10_10[iaa_positive_all_10_10['gtdb_tax'].notnull()]
# iaa_positive_all_10_10.to_pickle(output_directory+"iaa_positive_all_data_10_10.pickle")
# iaa_positive_all_10_10.to_csv(output_directory+"iaa_positive_all_data_10_10.tsv", sep = '\t', index = False)
# iaa_positive_all_10_10.to_excel(output_directory+"iaa_positive_all_data_10_10.xlsx", index = False)
# iaa_positive_all_gtdb_10_10.to_pickle(output_directory+"iaa_positive_all_data_gtdb_10_10.pickle")
# iaa_positive_all_gtdb_10_10.to_csv(output_directory+"iaa_positive_all_data_gtdb_10_10.tsv", sep = '\t', index = False)
# iaa_positive_all_gtdb_10_10.to_excel(output_directory+"iaa_positive_all_data_gtdb_10_10.xlsx", index = False)
# #get 20 neighbors on each side of the cluster:
#
# inputs_fetchneighborhood_20_20 = list(range(0,len(iaa_positive_df)))
...
#         pass
#
# iaa_pos_neighborhoods_20_20 = list(filter(None, outputs_fetchneighborhood_20_20))
# iaa_pos_neighborhoods_df_20_20= pd.DataFrame(iaa_pos_neighborhoods_20_20, columns=fetchneighborhood_columns)
# iaa_pos_neighborhoods_df_20_20.to_csv(output_directory+"iaa_pos_neighborhoods_20_20.tsv",sep = '\t', index = False)
# iaa_pos_neighborhoods_df_20_20.to_pickle(output_directory+"iaa_pos_neighborhoods_20_20.pickle")
# iaa_pos_neighborhoods_df_20_20 = pd.read_pickle(output_directory+"iaa_pos_neighborhoods_20_20.pickle")
# iaa_positive_all_20_20 = pd.merge(iaa_pos_neighborhoods_df_20_20,iaa_positive_df, on = ["accession","cluster_number","assembly"])
# iaa_positive_all_20_20 = iaa_positive_all_20_20.rename(columns={"coord_list_y": "coord_list"}).drop(columns=['coord_list_x'])
# iaa_positive_all_gtdb_20_20= iaa_positive_all_20_20[iaa_positive_all_20_20['gtdb_tax'].notnull()]
# iaa_positive_all_20_20.to_pickle(output_directory+"iaa_positive_all_data_20_20.pickle")
# iaa_positive_all_20_20.to_csv(output_directory+"iaa_positive_all_data_20_20.tsv", sep = '\t', index = False)
# iaa_positive_all_20_20.to_excel(output_directory+"iaa_positive_all_data_20_20.xlsx", index = False)
# iaa_positive_all_gtdb_20_20.to_pickle(output_directory+"iaa_positive_all_data_gtdb_20_20.pickle")
# iaa_positive_all_gtdb_20_20.to_csv(output_directory+"iaa_positive_all_data_gtdb_20_20.tsv", sep = '\t', index = False)
# iaa_positive_all_gtdb_20_20.to_excel(output_directory+"iaa_positive_all_data_gtdb_20_20.xlsx", index = False)
