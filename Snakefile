...
rule all:
    input:
        ...
        "gtdb/bac120_metadata_r89.tsv",
        "results/iac_positive_all_data.xlsx",
        "results/iac_positive_all_data_gtdb.xlsx",
        "results/IacD_seqs.fa",
        "results/IacD_seqs.fa.trim.msa.fasttree.tree"

rule download_gbff_files:
    params:
        path=expand(i for i in all_paths),
        name=expand(i for i in sample_names)
    output: expand("gbff_files/{sample}.gbff.gz", sample=sample_names)
    run:
        import concurrent.futures
        import psutil
        import hashlib
        with open("ftp_paths.txt", 'r+') as file:
                 ftp_paths= file.read().splitlines()
        def fetch_genome(path, retries=20):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=number_of_cpus*2) as executor:
            for _ in executor.map(fetch_genome, params.path):
                pass
...
rule unzip_gbff_files:
    ...
    run:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=None) as executor:
            for _ in executor.map(unzip_genome, input):
                pass

rule parse_gbff_files:
    input: expand("gbff_files_unzipped/{sample}.gbff", sample=sample_names)
    output: expand("fasta_files/{sample}_proteins.fa", sample=sample_names)
    run:
        import re
        import textwrap
        import multiprocessing as mp
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=None) as executor:
            for _ in executor.map(gbff2protein, input):
                pass
        # pool = mp.Pool(mp.cpu_count()-2)
        # pool_output = pool.map(gbff2protein, list(input))
        # pool.close()

rule combine_fasta:
    input: rules.parse_gbff_files.output
    # input: expand("fasta_files/{sample}_proteins.fa", sample=sample_names)
    output: expand("fasta_files_combined/all_proteins_combined_{db_id}.fa", db_id= combined_fasta_chunks)
    run:
        import glob
        import concurrent.futures
        import psutil
        ...
        def combine_files(file_list_index):
            with open('fasta_files_combined/all_proteins_combined_'+str(file_list_index)+".fa", 'w') as outfile:
                files = fasta_file_chunks[file_list_index]
                for f in files:
                        with open(f) as infile:
                                outfile.write(infile.read())
        with concurrent.futures.ThreadPoolExecutor(max_workers=number_of_cpus*2) as executor:
            for _ in executor.map(combine_files, range(len(fasta_file_chunks))):
                pass
...
rule make_master_db:
    input:
        expand("fasta_files_combined/all_proteins_combined_{db_id}.fa.phr", db_id=combined_fasta_chunks),
        expand("fasta_files_combined/all_proteins_combined_{db_id}.fa.pin", db_id=combined_fasta_chunks),
        expand("fasta_files_combined/all_proteins_combined_{db_id}.fa.psq", db_id=combined_fasta_chunks),
...
rule parse_blastp:
    input: rules.add_header_to_blast_out.output
    output:
        "results/iac_positive_accessions.txt",
        "results/iac_positive_df.tsv"
    run:
...
rule index_files:
    input: "results/iac_positive_df.pickle"
...
rule fetch_neighborhoods:
    ...
    output: "results/iac_positive_all_data.xlsx", "results/iac_positive_all_data_gtdb.xlsx"
    run:
    ...
    run:
        ...
        for i in range(len(gtdb['accession'])):
            ...
            for n in range(len(indices)):
                ...
                protein_info.append(gtdb['assembly_x'].iloc[i]+"_"+ast.literal_eval(gtdb['nhbrhood_hit_list'].iloc[i])[index]+"_n"+str(n+1)+"_"+str(gtdb['species_gtdb'].iloc[i])+"_"+ast.literal_eval(re.sub('nan',"'absent'",gtdb['nhbrhood_locus_tags'].iloc[i]))[index]+"_"+ast.literal_eval(re.sub('nan',"'absent'",gtdb['nhbrhood_prot_ids'].iloc[i]))[index])
        ...
        for i,j in zip(protein_info,protein_seqs):
            if re.search('IacD_n1_s__Sphingomonas',i):
                ...
                fasta_out+=i+'\n'+j+'\n\n'
        with open("results/IacD_seqs.fa",'w+') as output:
            output.writelines(fasta_out)

rule analyze_iacD:
    input: "results/IacD_seqs.fa"
    output: "results/IacD_seqs.fa.trim.msa.fasttree.tree"
    shell:
        ...
        #     url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi?dbfrom=nuccore&db=biosample&id=" + i + "&api_key=52553dfd9c090cfba1c3b28a45d8a648fd09"
        #     buffer = BytesIO()
        #     c = pycurl.Curl()
        #     c.setopt(c.URL, url)
        #     c.setopt(c.WRITEDATA, buffer)
        ...
        # iac_positive_df['biosample'] = iac_positive_df['accession'].map(biosample_ids_dict)
...
# rule download_refseq_summary:
#     output: "assembly_summary.txt"
#     run:
#         import urllib.request
#         url = 'ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/bacteria/assembly_summary.txt'
#         urllib.request.urlretrieve(url,"assembly_summary.txt")
# #
# checkpoint filter_refseq_summary:
#     input:"assembly_summary.txt"
#     output:
#         a="assembly_summary_filtered.txt",
#         b="ftp_paths.txt",
#         c="sample_names.txt"
#     run:
#         import pandas as pd
...
#         with open("ftp_paths.txt", 'r+') as file:
#                  ftp_paths= file.read().splitlines()
#         sample_names=pd.DataFrame([re.search("GCF_.+genomic(?=.gbff.gz)?",i.split('/')[-1])[0] for i in ftp_paths])
#         sample_names.to_csv("sample_names.txt", sep='\t', index=False, header=False)
#
#
# rule download_data:
#     input:
#         "ftp_paths.txt"
#     output:"gbff_files/{sample_id}"
#     run:
#         # with open(input[0], 'r') as fh:
#         #     for l in fh:
#         #         l = l.rstrip()
#         #         # pretend this downloads data
#         #         with open("work_dir/"+l,'w'):
#         #             print(l)
#         import concurrent.futures
#         import psutil
...
#         number_of_cpus = psutil.cpu_count()
#         with concurrent.futures.ThreadPoolExecutor(max_workers=number_of_cpus*2) as executor:
#             for _ in executor.map(fetch_genome, ftp_paths):
#                 pass
#
# def aggregate_signatures(wildcards):
#     checkpoint_output = checkpoints.filter_refseq_summary.get(**wildcards).output[1]
#     samples= list(pd.read_csv(checkpoint_output))
#     samples= [i.split('/')[-1] for i in samples]
#     print(checkpoint_output)
#     return expand("gbff_files/{sample_id}", sample_id= samples)
#
# rule make_database:
...
# rule print_data:
#     input: rules.filter_refseq_summary.output.b
#     output:
#         "results.txt"
#     params: [i for i in pd.read_csv(rules.filter_refseq_summary.output.b)]
#     run:
#         print(params)
#
        # import urllib.request
